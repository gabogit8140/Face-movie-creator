...
import { ProcessedImage, Point, TransitionStyle } from '../types';
import { PlayIcon, PauseIcon, DownloadIcon, UploadIcon } from './icons';
...
interface MoviePreviewProps {
  ...
  onAngleOffsetChange: (offset: number) => void;
}
...
const MoviePreview: React.FC<MoviePreviewProps> = ({ 
    ...
    const drawFrame = useCallback((frameTime: number) => {
        ...
        const drawTextOverlay = (img: ProcessedImage, timeIntoImage: number) => {
        ...
        const drawImageCentered = (
            img: ProcessedImage, 
            opacity: number,
            transform?: { scale?: number; pan?: Point }
        ) => {
            ...
            const pupilCenter: Point = { x: (leftPupil.x + rightPupil.x) / 2, y: (leftPupil.y + rightPupil.y) / 2 };
            const pupilAngle = Math.atan2(rightPupil.y - leftPupil.y, rightPupil.x - leftPupil.x);
    
            const TARGET_CENTER: Point = { x: canvas.width / 2, y: canvas.height * 0.4 + faceVerticalOffset };
    ...
    const runAnimation = useCallback((now: number) => {
    ...
    const generateVideo = useCallback((format: 'webm' | 'mp4') => new Promise<void>((resolve) => {
        const canvas = canvasRef.current;
        if (!canvas) {
            resolve();
            return;
        ...
        let recordAnimationId: number | null = null;
    ...
    const generateGif = useCallback(() => new Promise<void>(async (resolve) => {
        const canvas = canvasRef.current!;
        const gif = new GIF({
            workers: 2,
            quality: 10,
    ...
    return (
        <div className="bg-gray-800 p-4 sm:p-6 rounded-lg shadow-lg">
            <h2 className="text-xl font-semibold mb-4 text-white">3. Preview & Download</h2>
            <div className="aspect-video w-full bg-gray-900 rounded-md overflow-hidden relative">
                <canvas 
                    ref={canvasRef} 
                ...
                {images.length === 0 && !isGenerating && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-gray-400 p-4">
                        ...
                        <h3 className="text-xl font-semibold text-gray-300">Your Movie Preview</h3>
                        <p className="mt-1">Add photos and select faces to see your movie here.</p>
                    </div>
                )}
                {!isPlaying && images.length > 0 && !isGenerating && (
                    <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                        <button onClick={handlePlayPause} className="text-white bg-purple-600 rounded-full p-4 hover:bg-purple-700 transition">
             ...
             <div className="mt-4 border-t border-gray-700 pt-6 space-y-4">
                <div className="flex items-center gap-4">
                    <label htmlFor="zoom-slider" className="text-sm font-medium text-gray-300 w-28 whitespace-nowrap">Face Size</label>
                    <input id="zoom-slider" type="range" min="0.1" max="2.5" step="0.1" value={faceZoom} onChange={(e) => onZoomChange(parseFloat(e.target.value))} className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer [&::-webkit-slider-thumb]:bg-purple-500 [&::-moz-range-thumb]:bg-purple-500" disabled={isGenerating}/>
                    <span className="text-sm font-mono text-gray-400 w-12 text-center">{faceZoom.toFixed(1)}x</span>
                </div>
                 <div className="flex items-center gap-4">
                    <label htmlFor="vertical-offset-slider" className="text-sm font-medium text-gray-300 w-28 whitespace-nowrap">Vertical Position</label>
                    <input id="vertical-offset-slider" type="range" min="-100" max="100" step="1" value={faceVerticalOffset} onChange={(e) => onVerticalOffsetChange(parseInt(e.target.value, 10))} className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer [&::-webkit-slider-thumb]:bg-purple-500 [&::-moz-range-thumb]:bg-purple-500" disabled={isGenerating}/>
                    <span className="text-sm font-mono text-gray-400 w-12 text-center">{faceVerticalOffset}px</span>
                </div>
                 <div className="flex items-center gap-4">
                    <label htmlFor="rotation-slider" className="text-sm font-medium text-gray-300 w-28 whitespace-nowrap">Rotation</label>
                    <input id="rotation-slider" type="range" min="-20" max="20" step="1" value={faceAngleOffset} onChange={(e) => onAngleOffsetChange(parseInt(e.target.value, 10))} className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer [&::-webkit-slider-thumb]:bg-purple-500 [&::-moz-range-thumb]:bg-purple-500" disabled={isGenerating}/>
                    <span className="text-sm font-mono text-gray-400 w-12 text-center">{faceAngleOffset}°</span>
                </div>
            </div>
            <div className="mt-6 flex flex-col sm:flex-row gap-4 items-center">
                 <button onClick={handlePlayPause} disabled={images.length < 1 || isGenerating} className="flex-1 sm:flex-none inline-flex items-center justify-center px-4 py-2.5 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 focus:ring-offset-gray-800 disabled:bg-gray-600 disabled:cursor-not-allowed">
                ...
                <div className="flex-grow w-full sm:w-auto">
                    <label htmlFor="format-select" className="sr-only">Export Format</label>
                    <select id="format-select" value={exportFormat} onChange={e => setExportFormat(e.target.value as any)} disabled={isGenerating} className="w-full bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block p-2.5">
                        <option value="webm">WebM Video</option>
                        <option value="mp4" disabled={!isMp4Supported}>MP4 Video {!isMp4Supported && "(Not Supported)"}</option>
                        <option value="gif">Animated GIF</option>
                    </select>
                </div>
            </div>
            <div className="mt-4">
                <button onClick={handleGenerateExport} disabled={isGenerating || images.length < 2} className="w-full inline-flex items-center justify-center px-4 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 focus:ring-offset-gray-800 disabled:bg-gray-600 disabled:cursor-not-allowed">
            ...
            {isGenerating && (
                <div className="mt-4 text-center text-gray-300">
                    <div className="w-full bg-gray-700 rounded-full h-2.5 mb-2">
                        <div className="bg-purple-600 h-2.5 rounded-full" style={{ width: `${generationProgress}%`, transition: 'width 0.1s' }}></div>
                    </div>
                    <p>Creating your masterpiece... This will take about {Math.round(images.length * 2)} seconds.</p>
                    {exportFormat === 'gif' && <p className="text-sm text-gray-400">GIFs can take longer. Please be patient.</p>}
                </div>
            )}
            
            {videoUrl && (
                <div className="mt-6 p-4 bg-gray-700 rounded-lg">
                    <h3 className="font-semibold text-lg text-green-400">Your movie is ready!</h3>
                    <div className="mt-2 flex items-center gap-4">
                         {exportFormat === 'gif' ? (
                            <img src={videoUrl} alt="Generated animated GIF" className="w-1/2 rounded-md" />
                        ) : (
                            <video src={videoUrl} controls className="w-1/2 rounded-md"></video>
                        )}
                        <a
                            href={videoUrl}
